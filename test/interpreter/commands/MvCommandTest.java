...
class MvCommandTest {
  ...
  void testMoveFileToDirectory() throws IOException {
    ...
    assertTrue(
        result.contains("Moved \"" + sourceFile.getPath() + "\" to directory \"" + destinationDir.getPath() + "\"."));
  ...
  void testMoveToInvalidDirectory() throws IOException {
    ...
    assertTrue(result.contains(
        "Error: Unable to move/rename \"" + sourceFile.getPath() + "\" to \"" + invalidDir.getPath() + "\"."));
  }
  ...
  void testMultipleNonExistentSourceFiles() {
    ...
    String result = mvCommand.moreThanTwoArguments(List.of("file1.txt", "file2.txt", destinationDir.getPath()),
        context);
  ...
  void testMixedExistingAndNonExistentSourceFiles() throws IOException {
    ...
    String result = mvCommand
        .moreThanTwoArguments(List.of("existingFile.txt", "nonExistentFile.txt", destinationDir.getPath()), context);

    // Assert that the output contains the correct success message for the existing
    // file
    assertTrue(result.contains("Moved\"" + existingFile.getPath() + "\" to \""
        + new File(destinationDir, existingFile.getName()).getPath() + "\""));
    // Assert that the output contains the error message for the nonexistent file
    assertTrue(result.contains("Error: Source file \"nonExistentFile.txt\" does not exist"));
  }
}
